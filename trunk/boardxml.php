...
header("Content-Disposition: inline; filename=boardxml.xml");
...
echo "<document>" . "\n";
echo "<row>\n<unit>unit</unit>\n<first>first</first>\n<last>last</last>\n<phone>phone</phone>\n<email>email</email>\n</row>" . "\n";
$q = "SELECT unit.name AS u, user.name AS n, user.phone AS p, user.email AS e FROM unit, user WHERE unit.id=user.unitid AND user.board=1 ORDER BY user.name ASC";
$r = query($q);
$count =0;
