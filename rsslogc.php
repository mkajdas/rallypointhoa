...
if($_SESSION['admin']==1 && is_numeric($_GET['id'])){
...
<?php
$q = "SELECT log.note AS n, log.dt AS d, user.name AS name, log.id AS id FROM log LEFT JOIN user ON log.userid=user.id WHERE log.id=" . $_GET['id'] . " ORDER BY dt DESC";
$r = query($q);
$count =0;
...
<?php
$query = "SELECT logcomment.note AS n, logcomment.dt AS d, user.name AS name, logcomment.id AS id FROM logcomment LEFT JOIN user ON logcomment.userid=user.id WHERE logcomment.noteid=" . $_GET['id']. " ORDER BY dt DESC";
$result = query($query);
while($row=fetch_array($result)){
$titlen = strpos($row['n'], "\n",1);
if($titlen===false){
	$title = xmlreplace(unescape_data($row['n']));
}else{
	$title = xmlreplace(substr(unescape_data($row['n']),0,$titlen));
}
?>
<item>
            <title><?=$title?></title>
            <link><?=$addressofwebsite?></link>
            <description><?=xmlreplace(createHTML($row['n']))?><?php if(strlen($row['name'])>0){?><?=xmlreplace("<br /><i>")?>posted by <?=$row['name']?><?=xmlreplace("</i>")?><?php } ?></description>
            <pubDate><?php if($row['d']>0){ echo date("r",$row['d']); }else{ echo "Fri, 29 Sep 2006 20:52:01 -0400"; } ?></pubDate>
</item>
<?php
}
}
