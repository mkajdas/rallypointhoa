...
<rss version="0.91">
    <channel>
        <title><?=$titleofsite?> Files</title>
        <description></description>
        <link><?=$addressofwebsite?></link>
        <lastBuildDate><?=date("r")?></lastBuildDate>
<?php
$q = "SELECT name, ext, board, t, owner, id, dt FROM file WHERE id=id" . $board . $owner ." ORDER BY name ASC";
$r = query($q);
$count =0;
while($rr=fetch_array($r)){
?><item>
            <title><?=xmlreplace(unescape_data($rr['name']))?></title>
            <link><?=$addressofwebsite?>showfile.php?id=<?=$rr['id']?></link>
            <description></description>
