...
<rss version="0.91">
    <channel>
        <title><?=$titleofsite?> - Finances for your Unit</title>
        <description></description>
        <link><?=$addressofwebsite?></link>
        <lastBuildDate><?=date("r")?></lastBuildDate>
<?php
$q = "SELECT * FROM financial WHERE unitid=" . $_SESSION['unitid']  . " ORDER BY dt DESC";
$r = query($q);
$count =0;
while($rr=fetch_array($r)){
$title = xmlreplace(unescape_data($rr['description'] . " - $" . number_format($rr['amount'],2)));
